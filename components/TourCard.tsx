...
import { getTranslatedTour, getTranslatedCategory } from '../data/translations';

const TourCard: React.FC<{ item: TourItem }> = ({ item }) => {
  const { language } = useLanguage();
  
  const texts = {
    pt: { website: 'Site Oficial', details: 'Detalhes' },
    en: { website: 'Official Website', details: 'Details' },
    es: { website: 'Sitio Oficial', details: 'Detalles' },
  };

  const t = texts[language];
  
  // Get translated content for this tour
  const translation = getTranslatedTour(item.id, language);
  const title = translation?.title || item.title;
  const description = translation?.description || item.description;
  const status = translation?.status || item.status;
  const translatedCategory = getTranslatedCategory(item.category, language);

  const hasSocialLinks = item.instagram || item.facebook;
    ...
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col h-full">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="mt-auto space-y-2 border-t pt-4 border-slate-100">
          ...
          {status && (
            <p className="text-red-500 text-[10px] font-bold uppercase italic">{status}</p>
          )}
        </div>
