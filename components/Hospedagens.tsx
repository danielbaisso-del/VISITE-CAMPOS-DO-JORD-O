...
const Card: React.FC<{ a: Accommodation; onDetails: (a: Accommodation) => void }> = ({ a, onDetails }) => (
  <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 flex flex-col">
    <div className="relative h-44 bg-slate-200">
      <img src={a.images?.[0] ?? 'https://picsum.photos/800/450'} alt={a.name} className="w-full h-full object-cover" />
    </div>
    <div className="p-4 flex flex-col flex-grow">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h4 className="text-lg font-semibold text-slate-800">{a.name}</h4>
      ...
      <p className="text-sm text-slate-600 mt-3 line-clamp-3">{a.description}</p>
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map(item => <Card key={item.id} a={item} onDetails={setSelected} />)}
      </div>
